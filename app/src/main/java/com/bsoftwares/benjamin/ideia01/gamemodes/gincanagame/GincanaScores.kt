...
class GincanaScores : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnGincanaScoresNext.setOnClickListener {
            if(isEverythingChecked()){
                ...
                val idRbSelecionadoDificuldade = rgPerguntasTipos.findViewById<RadioButton>(rgPerguntasTipos.checkedRadioButtonId)
                val posicaoRbSelecionadoDificuldade = rgPerguntasTipos.indexOfChild(idRbSelecionadoDificuldade)
                val rbSelecionadoDificuldade = rgPerguntasTipos.getChildAt(posicaoRbSelecionadoDificuldade) as RadioButton
                ...
                if (todosRespondem){
                    ...
                    val idRbSelecionadoTime = rgTimesButtons.findViewById<RadioButton>(rgTimesButtons.checkedRadioButtonId)
    ...
    private fun definirPergunta(perguntas : java.util.ArrayList<QuestionParcelable>?, perguntaView : Dialog,pontos : Int) {
        perguntaView.txtPerguntaGincana.text = perguntas!![0].question
        perguntaView.btnReferencia.setOnClickListener {
            Toast.makeText(context,perguntas[0].reference,Toast.LENGTH_LONG).show()
        }
        val checkBoxes = Arrays.asList(perguntaView.btnResposta01,perguntaView.btnResposta02,perguntaView.btnResposta03,perguntaView.btnResposta04)
        if(todosRespondem){
            val opcoes = Arrays.asList(perguntas[0].correctAnswer,perguntas[0].answerB,perguntas[0].answerC,perguntas[0].answerD)
            ...
            perguntaView.btnChecarScores.setOnClickListener {
                ...
                alertDialog.setButton(AlertDialog.BUTTON_NEGATIVE,"Não") { dialog, which -> dialog.dismiss() }
                alertDialog.show()
            }
        }else{
            val opcoes = Arrays.asList(perguntas[0].correctAnswer,perguntas[0].answerB,perguntas[0].answerC,perguntas[0].answerD)
            ...
            perguntaView.btnChecarScores.setOnClickListener {
                if(respostaSelecionado == ""){
                    Toast.makeText(context,"É necessário escolhar uma resposta",Toast.LENGTH_SHORT).show()
                }else{
                    val alertDialog = AlertDialog.Builder(context).create()
                    alertDialog.setTitle(getString(R.string.confirmacao))
                    alertDialog.setMessage(getString(R.string.confirmarTime))
